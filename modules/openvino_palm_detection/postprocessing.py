...
class Postprocessor:
    ...
    def decode_predictions(self, predictions, output_names):
        out_clf = self._sigm(predictions[output_names[0]])[0].squeeze(-1)
        out_reg = predictions[output_names[1]].squeeze(0)
        keypoints = out_reg[:, 4:] * np.tile(self.anchors[:, 2:4], 7) / 256 + np.tile(
            self.anchors[:, 0:2], 7,
        )
        bboxes = np.empty((out_clf.shape[0], 5))
        x_center = out_reg[:, 0] / 256 + self.anchors[:, 0]
        y_center = out_reg[:, 1] / 256 + self.anchors[:, 1]
        w = out_reg[:, 2] / 256
        h = out_reg[:, 3] / 256
